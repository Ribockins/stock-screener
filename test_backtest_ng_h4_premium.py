"""
Backtester для NG (Natural Gas) H4 - PREMIUM сигналы за год
...
class NGH4PremiumBacktester:
    ...
    def download_ng_data(self, days=365):
        """Скачать данные NG за последний год"""
        logger.info(f"📥 Загружаю NG данные H4 за {days} дней...")
        ...
        try:
            ...
            df = yf.download('NG=F', start=start_date, end=end_date, interval='4h', progress=False)
    ...
    def check_signal_profitability(self, df, signal, lookforward=6):
        ...
        if signal['type'] == 'BEARISH':
            ...
            is_profitable = profit > 0.01  # Минимум 1 пункт профита
    ...
    def backtest(self):
        """Запустить бэктест"""
        print("\n" + "="*100)
        print("🧪 BACKTESTING NG (Natural Gas) H4 - PREMIUM Signals Only (1 Year)")
        print("="*100 + "\n")
        
        # Загружаем данные
        df = self.download_ng_data(days=365)
        if df is None:
        ...
        print("-" * 100)
        print(f"{'Date':<20} {'Price':<10} {'RSI':<8} {'Type':<10} {'Factors':<8} {'Confidence':<12} {'Profitable':<12} {'P/L (pts)':<12}")
        print("-" * 100)
        ...
        for signal in signals:
            ...
            if is_profitable is not None:
                ...
                print(f"{signal_date:<20} {signal['price']:<10.2f} {signal['rsi']:<8.1f} {signal['type']:<10} {signal['factors']:<8} {confidence:<12.1f}% {status:<12} {profit:<12.4f}")
        ...
        # Выводим результаты
        print("\n" + "="*100)
        print("📊 ИТОГОВЫЕ РЕЗУЛЬТАТЫ - PREMIUM SIGNALS ONLY")
        print("="*100)
        ...
        print(f"💰 Общий P/L за год (points):       {total_pnl:.4f}")
        ...
        print("-" * 100)
        print(f"{'Month':<12} {'Signals':<10} {'Wins':<8} {'Losses':<8} {'Win Rate':<12} {'P/L (pts)':<12}")
        print("-" * 100)
        
        for month in sorted(monthly_stats.keys()):
            stats = self.results['monthly_stats'][month]
            print(f"{month:<12} {stats['signals']:<10} {stats['wins']:<8} {stats['losses']:<8} {stats['win_rate']:<12}% {stats['pnl']:<12.4f}")
        
        print("-" * 100)
        print(f"🏆 Лучший месяц:  {best_month} (P/L: {best_pnl:.4f} pts)")
        print(f"📉 Худший месяц:  {worst_month} (P/L: {worst_pnl:.4f} pts)")
        print("="*100 + "\n")
    ...
    def save_results(self):
        ...
        with open('backtest_ng_h4_premium_results.json', 'w') as f:
            json.dump(self.results, f, indent=2)
        logger.info("💾 Результаты сохранены в backtest_ng_h4_premium_results.json")


if __name__ == '__main__':
    backtester = NGH4PremiumBacktester()
    backtester.backtest()
