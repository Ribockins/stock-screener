# Cross-market sequence report

_Regenerate: `python scripts/cross_market_profile_batch.py` → also writes `reports/cross_market_sequence_report.md` (gitignored)._

_Sierra uploads · peak hours local TZ · 30m fade retrace @ anchors_

## Data quality (latest)

| Symbol | Bar | Days | Calendar | Peak hours |
|--------|-----|------|----------|------------|
| **FRA40** | 1m | 21 | Jun–Jul 2026 | 15:00, 14:00, 16:00 |
| **UK100** | 1m | 21 | Jun–Jul 2026 | 14:00, 15:00, 16:00 |
| **US30 / SPX500** | 1m | 27 | Jun–Jul 2026 | 14:00–16:00 |
| **NGAS** | 5m | 27 | Jun–Jul 2026 | 14:00, 13:00 |
| **XAUUSD** | 1m | 27 | Jun–Jul 2026 | 14:00, 15:00, 02:00 |
| **XAGUSD** | 1m | 27 | Jun–Jul 2026 | 15:00, 14:00, 16:00 |
| **UKOil (Brent)** | 1m | 22 | Jun–Jul 2026 | 14:00, 15:00, 16:00 |
| **USOil (WTI)** | 1m | 27 | Jun–Jul 2026 | 14:00, 15:00, 16:00 |
| **GER30 / EUSTX50 / ESP35** | H1/1m | 22 | Jan–Feb 2026 | EU afternoon |

**Still missing:** **VOLX 1m**, CORNF/DJFXJPY re-upload.

**Daily macro:** `docs/cross_market_daily_report.md` · `config/macro_bias.json`

## Anchor retrace — FRA40 & UK100 (1m, Jun–Jul 2026)

| Symbol | Anchor | Med retrace | ≥50% | GEM note |
|--------|--------|-------------|------|----------|
| **FRA40** | **11:00** | **66.7%** | 52% | Primary EU morning fade |
| **FRA40** | **17:30** | **71.7%** | 57% | US overlap — best CAC window |
| FRA40 | 15:30 | 32.0% | 48% | Skip blind fade |
| **UK100** | **11:00** | **52.8%** | 52% | FTSE morning fade |
| UK100 | 15:30 | 38.5% | 48% | Weak |
| UK100 | 17:30 | 24.4% | 38% | Avoid |

## Lead–lag (15m, same calendar where possible)

| Pair | Lag | Corr | Read |
|------|-----|------|------|
| **UK100→FRA40** | FRA40 ~60m ahead | 0.52 | Watch CAC for UK direction |
| UK100→US30 | US30 ~60m ahead | 0.10 | Weak — use for NY handoff only |
| US30→SPX500 | sync | 0.72 | US cluster |
| NGAS→US30 | US30 ~45m ahead | 0.03 | NGAS not a US leader |

## US30 (1m, refreshed 2026-07-24)

| Anchor | Med retrace | ≥50% | Note |
|--------|-------------|------|------|
| **14:30** | **64.6%** | 62% | NY cluster anchor |
| 09:30 | 10.0% | 38% | Continuation — no fade |
| 16:00 | 6.1% | 33% | Avoid fade |

## XAUUSD (gold 1m)

| Anchor | Med retrace | ≥50% | Note |
|--------|-------------|------|------|
| **14:30** | **62.6%** | 57% | Trade with US30/SPX cluster |
| 09:30 | 14.1% | 38% | Skip fade |
| 16:00 | −19.7% | 38% | Continuation — avoid fade |

**15m corr:** XAUUSD↔US30 **0.37**, XAUUSD↔SPX500 **0.40** — synchronous risk-on leg at NY peak.

## XAGUSD (silver 1m)

| Anchor | Med retrace | ≥50% | Note |
|--------|-------------|------|------|
| **16:00** | **51.7%** | 52% | Best silver-only window |
| 14:30 | 42.1% | 48% | Only with XAUUSD/US30 GEM confirm |
| 09:30 | 23.2% | 38% | Skip |

**15m corr:** XAGUSD↔XAUUSD **0.86** — trade silver as **follow leg** to gold.

## UKOil / Brent (1m)

| Anchor | Med retrace | ≥50% | Note |
|--------|-------------|------|------|
| **16:00** | **52.5%** | 52% | Primary Brent fade |
| 14:30 | 36.7% | 48% | Not US index cluster — use NGAS confirm |
| 09:30 | 36.7% | 43% | Weak |

**15m corr:** UKOil↔NGAS **0.24** (sync); UKOil↔US30 **0.05** (decoupled at NY peak).

## USOil / WTI (1m)

| Anchor | Med retrace | ≥50% | Note |
|--------|-------------|------|------|
| **16:00** | **56.2%** | 52% | Primary WTI fade (best oil window) |
| 14:30 | 28.5% | 33% | Avoid blind fade |
| 09:30 | 32.1% | 33% | Weak |

**15m corr:** USOil↔UKOil **0.90** (sync — spread trades only on divergence); USOil↔NGAS **0.21**.

## Sequence (GEM + VECTOR)

1. **London 11:00** — UK100 fade; confirm with FRA40 (CAC often leads UK by ~1h on 15m).
2. **Paris 17:30** — FRA40 fade (strongest CAC anchor in sample).
3. **NY 14:30** — US30/SPX500/NGAS/**XAUUSD** cluster; **UKOil** only with NGAS GEM; **XAGUSD** with gold/US confirm.
4. **NY 16:00** — **USOil** + **UKOil** + **XAGUSD** energy/metals fade block (WTI strongest at 56% med).
5. **Macro filter** — yesterday US30 ≤−1% → mild oil bid next day (`macro_bias.json`).
6. **GEM CONFIRMED** + **VECTOR** to profile `exit_et`.
